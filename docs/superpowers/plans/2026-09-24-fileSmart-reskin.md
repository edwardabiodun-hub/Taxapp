...
- [ ] Run: `grep -rn "rounded-2xl\|TaxEase" src/ index.html capacitor.config.ts package.json` — expect zero matches (confirms both the radius sweep and the rename are complete; `src/lib/local-db.ts`'s `TaxEaseAfrica` Dexie name is the one deliberate, documented exception — if it shows up in this grep, that's expected and correct, not a miss).
